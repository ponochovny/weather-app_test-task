  ...
  const handleSubmit = (e) => {
    ...
    fetchData(formData)
			.then(data => {
				...
				dispatch({type: 'NEW_ITEM', payload: {...obj, main: listOfCities.length === 0}})
      })
  }
    ...
    <div className="AddCoordinatesForm">
      ...
      <form onSubmit={handleSubmit}>
        <div className="fieldset">
          <label htmlFor="">
            <p>Longitude</p>
            <input type="text" value={formData.lon} onChange={(e) => setFormData({lat: formData.lat, lon: +e.target.value})} />
          </label>
        </div>
        <div className="fieldset">
          <label htmlFor="">
            <p>Latitude</p>
            <input type="text" value={formData.lat} onChange={(e) => setFormData({lat: +e.target.value, lon: formData.lon})} />
          </label>
        </div>
        <button>Add</button>
      </form>
